            ...
            try
            {
                ...
                while (!defeated)
                {
                    foreach (var attacker in characters)
                    {
                        ...
                        if (userWeapon && attacker.Weapon != null)
                        {
                            attackUsed = attacker.Weapon.Name;
                            damage = DoWeaponAttack(attacker, opponent);
                        }
                        else if (!userWeapon && attacker.Skills != null)
                        {
                        ...
                        {
                            response.Data.Log.Add($"{attacker.Name} has no weapon or skills!");
                            continue;
                        }

                        response.Data.Log.Add($"{attacker.Name} attacks {opponent.Name} with {attackUsed} for {(damage >= 0 ? damage : 0)} damage!");
                        ...
                        if (opponent.HitPoints <= 0)
                        {
                            ...
                            response.Data.Log.Add($"{attacker.Name} wins with {attacker.HitPoints} health left!");
                            break;
                        }
                    }
                }
